...
const orderCheckout = async (req, res) => {
  try {
    ...
    const addresses = await addressModel.findOne({ user: req.session.userId });
    ...
    const cartCheckout = await cartModel.findOne({ owner: req.session.userId });
        ...
        console.log(userDetails,"ORDERPOST")
    ...
    console.log(userDetails,"After order")
...
const orderConfirmation = async (req, res) => {
  ...
  try {
    ...
    await cartModel.findOneAndUpdate({owner:req.session.userId},{
      $set:{
        items:[]
      }
    })
